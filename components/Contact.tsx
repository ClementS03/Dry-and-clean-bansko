'use client'

import { useLanguage } from '@/context/LanguageContext'
import { useScrollReveal } from '@/hooks/useScrollReveal'

export default function Contact() {
  const { t } = useLanguage()
  const c = t.contact
  const ref = useScrollReveal()

  const waUrl = `https://wa.me/${c.whatsappNumber}?text=${encodeURIComponent(c.whatsappMsg)}`

  return (
    <section id="contact" className="section-pad bg-ink relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-gold/30 to-transparent" />
      {/* Full-width gold glow */}
      <div
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[80vw] h-[50vh] opacity-[0.06] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at top, #F5C400, transparent 65%)' }}
      />

      <div ref={ref} className="max-w-5xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <div className="text-center mb-14 reveal">
          <h2
            className="font-display text-4xl sm:text-5xl text-cream uppercase tracking-tight"
            style={{ fontFamily: 'Oswald, sans-serif' }}
          >
            {c.title}
          </h2>
          <p className="text-cream/50 mt-3 text-sm">{c.subtitle}</p>
        </div>

        <div className="grid md:grid-cols-2 gap-8">
          {/* ── Left: contact info ── */}
          <div className="reveal space-y-5">
            {/* Phone EN */}
            <a
              href={`tel:${c.phoneEN.replace(/\s/g, '')}`}
              className="flex items-center gap-4 p-5 card-dark group"
            >
              <span className="text-2xl">🇬🇧</span>
              <div className="flex-1">
                <div className="text-cream/40 text-xs uppercase tracking-widest mb-0.5">{c.phoneENLabel}</div>
                <div
                  className="font-display text-xl text-cream group-hover:text-gold transition-colors duration-200"
                  style={{ fontFamily: 'Oswald, sans-serif' }}
                >
                  {c.phoneEN}
                </div>
              </div>
              <svg className="w-4 h-4 text-gold/40 group-hover:text-gold transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
              </svg>
            </a>

            {/* Phone BG */}
            <a
              href={`tel:${c.phoneBG.replace(/\s/g, '')}`}
              className="flex items-center gap-4 p-5 card-dark group"
            >
              <span className="text-2xl">🇧🇬</span>
              <div className="flex-1">
                <div className="text-cream/40 text-xs uppercase tracking-widest mb-0.5">{c.phoneBGLabel}</div>
                <div
                  className="font-display text-xl text-cream group-hover:text-gold transition-colors duration-200"
                  style={{ fontFamily: 'Oswald, sans-serif' }}
                >
                  {c.phoneBG}
                </div>
              </div>
              <svg className="w-4 h-4 text-gold/40 group-hover:text-gold transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
              </svg>
            </a>

            {/* Area & hours */}
            <div className="p-5 card-dark space-y-3">
              <div className="flex items-center gap-3">
                <span className="text-xl">📍</span>
                <span className="text-cream/60 text-sm">{c.area}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xl">🕐</span>
                <span className="text-cream/60 text-sm">{c.hours}</span>
              </div>
            </div>
          </div>

          {/* ── Right: CTA buttons ── */}
          <div className="reveal flex flex-col gap-4 justify-center" style={{ transitionDelay: '150ms' }}>
            {/* WhatsApp CTA — primary */}
            <a
              href={waUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-3 px-6 py-5 rounded-sm text-white font-display text-lg uppercase tracking-wider transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(37,211,102,0.35)]"
              style={{
                background: 'linear-gradient(135deg, #25D366 0%, #1ebe5c 100%)',
                fontFamily: 'Oswald, sans-serif',
              }}
            >
              {/* WhatsApp icon */}
              <svg className="w-6 h-6 fill-white" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              {c.waBtn}
            </a>

            {/* Call CTA */}
            <a
              href={`tel:${c.phoneEN.replace(/\s/g, '')}`}
              className="btn-gold flex items-center justify-center gap-3 px-6 py-5 text-base"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
              </svg>
              {c.callBtn}
            </a>

            {/* Reassurance */}
            <p className="text-center text-cream/35 text-xs">
              {c.reassurance}
            </p>
          </div>
        </div>
      </div>
    </section>
  )
}
